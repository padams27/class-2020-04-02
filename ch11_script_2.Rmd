...
county_bootstrap <- rep_sample_n(county, size = nrow(county), reps = 100, replace = TRUE) %>% 
  group_by(replicate) %>% 
  nest()

x <- county_bootstrap %>% mutate(distinct_rows = map_dbl(data, ~ n_distinct(.)))
...
s10 <- x %>% mutate(mod = map(data, ~ lm(poverty ~ less_than_hs, data = .))) %>% 
